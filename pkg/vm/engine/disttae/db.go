...
func (e *Engine) getOrCreateSnapPart(
	ctx context.Context,
	tbl *txnTable,
	ts types.TS) (*logtailreplay.PartitionStateWithTombstoneObject, error) {
	...
	err = snap.ConsumeSnapCkps(ctx, ckps, func(
		...
		state *logtailreplay.PartitionStateWithTombstoneObject) error {
		locs := make([]string, 0)
...
func (e *Engine) LazyLoadLatestCkp(
	...
	if err := part.ConsumeCheckpoints(
		ctx,
		func(checkpoint string, state *logtailreplay.PartitionStateWithTombstoneObject) error {
			entries, closeCBs, err := logtail.LoadCheckpointEntries(
				ctx,
				e.service,
				checkpoint,
