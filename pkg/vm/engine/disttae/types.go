...
type Transaction struct {
	...
	// record the table created and dropped in the same txn,
	// for filtering effortless inserts when committing.
	tablesInVain []tableOp
...
func (txn *Transaction) PPString() string {
	...
	stringifySyncMap := func(m *sync.Map) string {
		...
		m.Range(func(key, _ interface{}) bool {
			...
			k := key.(tableKey)
			buf.WriteString(fmt.Sprintf("%v-%v-%v", k.accountId, k.dbName, k.name))
			i++
			return true
		})
		buf.WriteRune('}')
	...
	return fmt.Sprintf("Transaction{writes: %v, batchSelectList: %v, tableOps:%v, tablesInVain: %v,  tableCache: %v,  toFreeBatches: %v, insertCount: %v, snapshotWriteOffset: %v, rollbackCount: %v, statementID: %v, offsets: %v, timestamps: %v}",
		...
		txn.tableOps.string(),
		stringifySlice(txn.tablesInVain, func(a any) string { return fmt.Sprintf("%v", a) }),
