...
func TestSnapshotGC(t *testing.T) {
	...
	err = db.DiskCleaner.GetCleaner().DoCheck()
	assert.Nil(t, err)
	dataObject, tombstoneObject := testutil.GetUserTablesInsBatch(t, tae.Opts.Fs, rele2.ID(), types.TS{}, viewSnapshot, db.Catalog)
...
func TestSnapshotCheckpoint(t *testing.T) {
	ioutil.RunPipelineTest(
		func() {
			...
			tae.ForceCheckpoint()
			tae.ForceCheckpoint()
			dataObject, tombstoneObject := testutil.GetUserTablesInsBatch(t, tae.Opts.Fs, rel1.ID(), types.TS{}, snapshot, db.Catalog)
...
func TestCKPCollectObject(t *testing.T) {
	ioutil.RunPipelineTest(
		func() {
			...
			collector := logtail.NewIncrementalCollector("", tae.Opts.Fs, types.TS{}, tae.TxnMgr.Now())
