...
func TestNonAppendableBlock(t *testing.T) {
...

	{
		...
		err = tables.HybridScan(context.Background(), tbl, txn, &view, schema, []int{2}, blkID, common.DefaultAllocator)
		assert.Nil(t, err)
		...
		err = tables.HybridScan(ctx, tbl, txn, &view, schema, []int{2}, blkID, common.DefaultAllocator)
		assert.Nil(t, err)
	...
	tae.Restart(ctx)
	{
		...
		for i := 0; it.Valid(); it.Next() {
			...
			for j := uint16(0); j < uint16(obj.BlkCnt()); j++ {
				...
				err := tables.HybridScan(ctx, tblEntry, txn, &views, schema, idxs, blkID, common.DefaultAllocator)
				require.NoError(t, err)
				defer views.Close()
...
func TestSystemDB1(t *testing.T) {
	...
	for it.Valid() {
		...
		blk := it.GetObject()
		err := blk.Scan(ctx, &bat, 0, idxs, common.DefaultAllocator)
		assert.NoError(t, err)
		defer bat.Close()
		t.Log(bat.String())
		it.Next()
...
func TestCompactblk3(t *testing.T) {
	...
	processor.ObjectFn = func(be *catalog.ObjectEntry) error {
		...
		for j := 0; j < be.BlockCnt(); j++ {
			...
			err := tables.HybridScan(ctx, tblEntry, txn, &view, schema, []int{0}, blkID, common.DefaultAllocator)
			assert.NoError(t, err)
			view.Compact()
...
func TestGetActiveRow(t *testing.T) {
	...
	_, rel = tae.GetRelation()
	{
		...
		it := rel2.MakeObjectIt(false, true)
		blk := it.GetObject().GetMeta().(*catalog.ObjectEntry)
...
func TestMergeBlocks3(t *testing.T) {
	...
	// 3. delete 8th row in blk2 and commit that after merging, test transfer
	{
		...
		// merge first Object
		objit := relm.MakeObjectIt(false, true)
		obj1 := objit.GetObject().GetMeta().(*catalog.ObjectEntry)
		require.NoError(t, err)
...
func TestAlterFakePk(t *testing.T) {
...

	{
		...
		var view *containers.Batch
		blkID := objectio.NewBlockidWithObjectID(obj.GetID(), 0)
		err = tables.HybridScan(ctx, tbl, txn, &view, newSchema.(*catalog.Schema), []int{1}, blkID, common.DefaultAllocator)
...
func TestReplayDeletes(t *testing.T) {
	...
	txn2, rel := tae.GetRelation()
	blkIt = rel.MakeObjectIt(true, true)
	blkEntry := blkIt.GetObject().GetMeta().(*catalog.ObjectEntry)
	txn, err := tae.StartTxn(nil)
	assert.NoError(t, err)
