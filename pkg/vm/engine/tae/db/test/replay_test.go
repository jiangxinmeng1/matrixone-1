...
func TestReplayCatalog1(t *testing.T) {
	...
	createTable := func(schema *catalog.Schema, wg *sync.WaitGroup, forceCkp bool) func() {
		return func() {
			...
			for i := 0; i < objCnt; i++ {
				stats := objectio.NewObjectStatsWithObjectID(objectio.NewObjectid(), false, false, false, false)
...
func TestReplay6(t *testing.T) {
	...
	schema.Extra.BlockMaxRows = 10
	schema.Extra.ObjectMaxBlocks = 2
	bat := catalog.MockBatch(schema, int(schema.Extra.BlockMaxRows*10-1))
	defer bat.Close()
	bats := bat.Split(4)
...
func TestReplay9(t *testing.T) {
	...
	schema.Extra.ObjectMaxBlocks = 2
	tae.BindSchema(schema)
	bat := catalog.MockBatch(schema, int(schema.Extra.BlockMaxRows*3+2))
	defer bat.Close()
	bats := bat.Split(4)
