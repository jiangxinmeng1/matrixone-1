...
func newLocalObject(table *txnTable) *tableSpace {
	return &tableSpace{
...
func (space *tableSpace) prepareApplyANode(node *anode) error {
	...
	for appended < node.Rows() {
		...
		if moerr.IsMoErrCode(err, moerr.ErrAppendableObjectNotFound) {
			segH, err := space.table.CreateObject(true)
			if err != nil {
				return err
			}
...
func (space *tableSpace) prepareApplyObjectStats(stats objectio.ObjectStats) (err error) {
	...
	shouldCreateNewSeg := func() bool {
		...
		entry := space.nseg.GetMeta().(*catalog.ObjectEntry)
		return !entry.ID.Eq(*sid)
	}

	if shouldCreateNewSeg() {
		space.nseg, err = space.table.CreateNonAppendableObject(true, new(objectio.CreateObjOpt).WithId(sid))
		if err != nil {
			return
		}
		space.nseg.GetMeta().(*catalog.ObjectEntry).SetSorted()
		err = space.nseg.UpdateStats(stats)
		if err != nil {
			return
		}
	}
	...
	for i := uint16(0); i < uint16(blkCount); i++ {
		...
		opts := new(objectio.CreateBlockOpt).
			...
			WithFileIdx(num).
			WithBlkIdx(i)
		_, err = space.nseg.CreateNonAppendableBlock(opts)
		if err != nil {
			return
		}
	}
